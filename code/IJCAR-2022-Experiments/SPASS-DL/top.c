  ...
  */ 
  ...
  while ((list_Empty(EmptyClauses) || !prfs_SplitStackEmpty(Search)) && 
    ...
    *BoundApplied = -1;
	...
	prfs_SetLoops(Search, 0); /* ATTENTION: Changed the semantics of bound loops: now stop after number of loops exceeded */
      }
      else
...
int main(int argc, const char* argv[])
{
  ...
  if (flag_GetFlagIntValue(Flags,flag_TIMELIMIT) != flag_TIMELIMITUNLIMITED) 
    alarm(flag_GetFlagIntValue(Flags,flag_TIMELIMIT)+30);  /* No race with main loop polling */
  ...
  do {
    ...
    if (flag_GetFlagIntValue(Flags, flag_TIMELIMIT) != 0 && top_NoAlarm == 0) {
      ...
      switch (Result) {
      case top_RESOURCE:
	if (flag_GetFlagIntValue(Flags,flag_TIMELIMIT) !=  flag_TIMELIMITUNLIMITED &&  flag_GetFlagIntValue(Flags,flag_TIMELIMIT) < clock_GetSeconds(clock_OVERALL))
